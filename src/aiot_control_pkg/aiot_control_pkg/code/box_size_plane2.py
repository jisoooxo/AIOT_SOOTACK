...
box_size_plane.py의 실험 버전: 평면적합(plane fit) 버전: 1차로 박스 전체를 단일 기준 z(z_center)로
윗면/OBB를 뽑은 뒤, 그 top 픽셀들의 (x, y, depth)에 평면 depth ≈ c0 + c1·x + c2·y 를 최소자승으로
맞춰서(옆면 등 잔차 큰 픽셀은 1회 걸러내고 재적합), 그 평면값을 픽셀별 기준 z로 삼아 2차로 재판정한다.
depth 카메라 특성상 평평한 윗면이라도 위치별 depth가 미세하게 기울어져 있어서, 단일 기준이나
계단식 3구역 기준으로는 긴 축 기울기를 다 못 따라가 박스 한쪽 끝이 TOL 밖으로 잘린다 - 평면으로
그 기울기 자체를 제거하는 게 목적.

box_size_plane.py와 다른 점: 2차(평면 피팅 후)에 closing/dilate-open/침식 같은 후처리는 생략하고,
실루엣 스냅(바닥-윗면 경계 복원)만 2차에 적용해서 그 결과를 그대로 최종으로 씀. 1차는 평면 피팅
재료 수집용이라 실루엣 스냅 없이 순수 tol 테스트 + closing/dilate-open만 함.
"""
...
TOL_LOW_Z  = 0.003     # z <= 3
TOL_Z_THRESH_CM = 3
TOL_LOW_Z0  = 0.003     # 3 < z <= 5 # 선풍기, 넓은 면은 0.003, 좁은 면은 0.0025
TOL_Z_THRESH_CM0 = 5
