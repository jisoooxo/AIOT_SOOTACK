...
class MotorControlNode(Node):
    ...
    def control_loop(self):
        ...
        error_each = np.abs(
            wrapped_q_delta(self.q_goal, q_actual)
        )
        ...
        if error > FINISH_TOLERANCE:
            self.get_logger().info(
                f'도착 대기: error={math.degrees(error):.2f}deg, '
                f'error_each={np.rad2deg(error_each).round(2).tolist()}, '
                f'actual={np.rad2deg(q_actual).round(1).tolist()}'
            )
            return
        ...
        self.get_logger().info(
            f'이동 완료: error={math.degrees(error):.2f}deg'
        )
    ...
    def control_waypoint_path(self):
        ...
        error_each = np.abs(wrapped_q_delta(self.q_goal, q_actual))
    ...
    def setup_motors(self):
        if not self.port_xh.openPort():
            raise RuntimeError(f'포트 열기 실패: {PORT_XH}')
        self.xh_open = True

        if not self.port_xm.openPort():
            raise RuntimeError(f'포트 열기 실패: {PORT_XM}')
        self.xm_open = True

        if not self.port_xh.setBaudRate(BAUDRATE):
            raise RuntimeError(f'Baudrate 설정 실패: {PORT_XH}')

        if not self.port_xm.setBaudRate(BAUDRATE):
            raise RuntimeError(f'Baudrate 설정 실패: {PORT_XM}')
    ...
    def q_to_raw(self, q):
        ...
        return [
            (
                int(HOME_RAW[i])
                + round(math.degrees(q[i]) * 4096.0 / 360.0)
            ) % 4096
            for i in range(DOF)
        ]
